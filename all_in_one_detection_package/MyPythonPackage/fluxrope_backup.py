...
def searchFluxRopeInWindow(B_DataFrame, VHT, n_theta_grid, minDuration, dt, flag_smoothA, savgol_filter_window):
...
def getResidueForCurrentAxial(theta_deg, phi_deg, minDuration, B_DataFrame, VHT, dt, flag_smoothA, savgol_filter_window):
    # Physics constants.
    mu0 = 4.0 * np.pi * 1e-7 #(N/A^2) magnetic constant permeability of free space vacuum permeability
    m_proton = 1.6726219e-27 # Proton mass. In kg.
    factor_deg2rad = np.pi/180.0 # Convert degree to rad.
...
def remove_discontinuity(data_DF, fluxRopeList_DF, **kwargs):
    # Default value.
    ...
    fluxRopeList_noDiscontinuity_DF = fluxRopeList_DF.copy()
    # Add 'discontinuity' column to fluxRopeList_DF.
    fluxRopeList_noDiscontinuity_DF = fluxRopeList_noDiscontinuity_DF.assign(Discontinuity=[-1.0]*len(fluxRopeList_noDiscontinuity_DF))
    print(fluxRopeList_noDiscontinuity_DF.keys())
    # Loop each record to calculate max Vsw discontinuity.
    recordLength = len(fluxRopeList_noDiscontinuity_DF)
    for index_temp, oneRecord_temp in fluxRopeList_noDiscontinuity_DF.iterrows():
        ...
        oneRecord_Vsw_temp.interpolate(method='time', limit=None, inplace=True)
        # interpolate won't fill leading NaNs, so we use backward fill.
        oneRecord_Vsw_temp.bfill(inplace=True)
        oneRecord_Vsw_temp.ffill(inplace=True)
        # Calculate Vsw magnitude.
        Vsw_temp = np.sqrt(np.square(oneRecord_Vsw_temp).sum(axis=1))
        # Calculate max Vsw jump.
        max_i_Vsw = len(Vsw_temp)-1 # max_i_Vsw is the max index.
        max_speedJump = 0.0
        for i_temp in xrange(1, max_i_Vsw): # Do not include the last element.
            if (i_temp-(smoothWindow-1)>=0)&(i_temp+smoothWindow<=max_i_Vsw):
                speedJump_temp = abs(np.nanmean(Vsw_temp[i_temp-(smoothWindow-1):i_temp+1]) - np.nanmean(Vsw_temp[i_temp+1:i_temp+smoothWindow+1]))
                if (speedJump_temp>max_speedJump):
                    max_speedJump = speedJump_temp
                    print('max_speedJump = {}'.format(max_speedJump))
            elif (i_temp-(smoothWindow-1)<0)&(i_temp+smoothWindow<=max_i_Vsw):
                speedJump_temp = abs(np.nanmean(Vsw_temp[:i_temp+1]) - np.nanmean(Vsw_temp[i_temp+1:i_temp+smoothWindow+1]))
                if (speedJump_temp>max_speedJump):
                    max_speedJump = speedJump_temp
                    print('max_speedJump = {}'.format(max_speedJump))
            elif (i_temp-(smoothWindow-1)>=0)&(i_temp+smoothWindow>max_i_Vsw):
                speedJump_temp = abs(np.nanmean(Vsw_temp[i_temp-(smoothWindow-1):i_temp+1]) - np.nanmean(Vsw_temp[i_temp+1:]))
                if (speedJump_temp>max_speedJump):
                    max_speedJump = speedJump_temp
                    print('max_speedJump = {}'.format(max_speedJump))
            elif (i_temp-(smoothWindow-1)<0)&(i_temp+smoothWindow>max_i_Vsw):
        ...
        # Save max_speedJump to fluxRopeList_noDiscontinuity_DF.
        fluxRopeList_noDiscontinuity_DF.loc[index_temp, 'Discontinuity']
    print(fluxRopeList_noDiscontinuity_DF)
'''
...
def remove_discontinuity(data_DF, fluxRopeList_DF, **kwargs):
    ...
    # Copy fluxRopeList_DF.
    fluxRopeList_with_std_diff_DF = fluxRopeList_DF.copy()
    # Get trimmed shockList.
    # Get time range from fluxropeList.
    startTime_all = fluxRopeList_with_std_diff_DF['startTime'].min()
    endTime_all = fluxRopeList_with_std_diff_DF['end'].max()
    shockList_trimmed_DF = shockList_DF[(shockList_DF.index>=startTime_all)&(shockList_DF.index<=endTime_all)]
    # Get the shock observed by specified spacecraft.
    ...
    # Add 'Vsw_std' and 'Vsw_diff' column to fluxRopeList_DF.
    fluxRopeList_with_std_diff_DF = fluxRopeList_with_std_diff_DF.assign(Vsw_std=[-1.0]*len(fluxRopeList_with_std_diff_DF))
    fluxRopeList_with_std_diff_DF = fluxRopeList_with_std_diff_DF.assign(Vsw_diff=[-1.0]*len(fluxRopeList_with_std_diff_DF))
    fluxRopeList_with_std_diff_DF = fluxRopeList_with_std_diff_DF.assign(containShock=[False]*len(fluxRopeList_with_std_diff_DF))
    
    # Loop each record to calculate Vsw_std and Vsw_diff.
    recordLength = len(fluxRopeList_with_std_diff_DF)
    for index_temp, oneRecord_temp in fluxRopeList_with_std_diff_DF.iterrows():
        print('\nindex_temp = {}/{}'.format(index_temp, recordLength))
        startTime_temp = oneRecord_temp['startTime']
        endTime_temp = oneRecord_temp['endTime']
        #duration = oneRecord_temp['duration']
        # Grab the data for one FR_record.
        selectedRange_mask = (data_DF.index >= oneRecord_temp['startTime']) & (data_DF.index <= oneRecord_temp['endTime'])
        # The data of fluxrope candidate.
        oneRecord_data_temp = data_DF.iloc[selectedRange_mask]
        ...
        # Check if contains shock.
        mask_shock_in_fluxRope = (shockList_trimmed_specifiedSpacecraft_DF['shockTime']>=startTime_temp)&(shockList_trimmed_specifiedSpacecraft_DF['shockTime']>=endTime_temp)
        if len(mask_shock_in_fluxRope).sum()>0
            fluxRopeList_with_std_diff_DF.loc[index_temp, 'containShock'] = True
        # Add std to fluxRopeList_with_std_diff_DF.
        fluxRopeList_with_std_diff_DF.loc[index_temp, 'Vsw_std'] = Vsw_temp.std(skipna=True)
        fluxRopeList_with_std_diff_DF.loc[index_temp, 'Vsw_diff'] = Vsw_diff_temp
        
        print(fluxRopeList_with_std_diff_DF[fluxRopeList_with_std_diff_DF['containShock']==True])
    
    return fluxRopeList_with_std_diff_DF
...
def clean_up_raw_result(data_DF, dataObject_or_dataPath, **kwargs):
    ...
    if isinstance(dataObject_or_dataPath, dict):
        ...
        search_result_raw = pickle.load(open(dataObject_or_dataPath, 'rb'))
    else:
    ...
    print('\nDefault parameters:')
    print('turnTime_tolerance    = {}'.format(turnTime_tolerance))
    ...
    if 'spacecraftID' in kwargs:
        spacecraftID = kwargs['spacecraftID']
        print('spacecraftID is {}.'.format(output_filename))
    ...
    for i_iteration in xrange(search_iteration):
        print('\n======================================================================')
        print('\ni_iteration = {}'.format(i_iteration+1))
        ...
        eventList_DF_2_fineResidue_temp = eventList_DF_1_fitSlot_temp[(eventList_DF_1_fitSlot_temp['residue_diff']<=0.12)&(eventList_DF_1_fitSlot_temp['residue_fit']<=0.16)]
        # Reset index
...
############################################        
        ...
        # Remove records with large Vsw fluctuations and with shocks.
        print('\nRemoving events with Vsw std greater than 18 and Vsw diff greater than 60...')
        if isVerbose+1:
            print('Before Vsw std check, total records is {}.'.format(len(eventList_DF_3_combinedByTurnTime_temp)))
        eventList_DF_3_combinedByTurnTime_temp = remove_discontinuity(data_DF, eventList_DF_3_combinedByTurnTime_temp)
        eventList_DF_3_combinedByTurnTime_temp = eventList_DF_3_combinedByTurnTime_temp[(eventList_DF_3_combinedByTurnTime_temp['Vsw_std']<18)&(eventList_DF_3_combinedByTurnTime_temp['Vsw_diff']<60)]
        # Drop 'Vsw_std'
        eventList_DF_3_combinedByTurnTime_temp = eventList_DF_3_combinedByTurnTime_temp.drop('Vsw_std', axis=1)
        # Drop 'Vsw_diff'
        eventList_DF_3_combinedByTurnTime_temp = eventList_DF_3_combinedByTurnTime_temp.drop('Vsw_diff', axis=1)
        # Reset index.
        eventList_DF_3_combinedByTurnTime_temp.reset_index(drop=True, inplace=True)
        if isVerbose+1:
            print('After Vsw std and diff check, total records is {}.'.format(len(eventList_DF_3_combinedByTurnTime_temp)))
        exit()
...
############################################
        ...
        if isVerbose:
            print('Before Walen test, total records is {}.'.format(len(eventList_DF_3_combinedByTurnTime_temp)))
        # Add an walen test result column.
        # .assign() always returns a copy of the data, leaving the original DataFrame untouched.
        eventList_DF_3_combinedByTurnTime_temp = eventList_DF_3_combinedByTurnTime_temp.assign(r = len(eventList_DF_3_combinedByTurnTime_temp)*[np.nan])
        # Cacluate walen test r value.
        eventList_DF_3_combinedByTurnTime_temp.reset_index(drop=True, inplace=True)
        len_eventList_DF_3_combinedByTurnTime_temp = len(eventList_DF_3_combinedByTurnTime_temp)
        for index, FR_record in eventList_DF_3_combinedByTurnTime_temp.iterrows():
            if isVerbose:
                print('Walen test: checking duration {} minutes, {}/{}...'.format(duration_str_temp, index+1, len_eventList_DF_3_combinedByTurnTime_temp))
            ...
            eventList_DF_3_combinedByTurnTime_temp.loc[index, 'r'] = round(walenTest_r_value, 4) # r.
            #print('r={}'.format(walenTest_r_value))

        # Remove the records with r>0.5.
        eventList_DF_4_passWalenTest_temp = eventList_DF_3_combinedByTurnTime_temp[abs(eventList_DF_3_combinedByTurnTime_temp['r'])<=0.5]
        # Drop 'r'
        ...
        if isVerbose:
            print('After Walen test, total records is {}.'.format(len(eventList_DF_4_passWalenTest_temp)))
        print('Done.')
        ...
        if eventList_DF_4_passWalenTest_temp.empty:
            # DataFrame eventList_DF_4_passWalenTest_temp is empty. Skip the rest operations in this loop.
        ...
        # Make a deep copy.
        eventList_DF_5_cleanedTurnTime_temp = eventList_DF_4_passWalenTest_temp.copy() # Default is deep copy.
        # Add difference of turn time column. Combine close turnTime.
        eventList_DF_5_cleanedTurnTime_temp = eventList_DF_5_cleanedTurnTime_temp.assign(diff=eventList_DF_5_cleanedTurnTime_temp['turnTime'].diff())
        eventList_DF_5_cleanedTurnTime_temp = eventList_DF_5_cleanedTurnTime_temp.assign(keepFlag=[True]*len(eventList_DF_5_cleanedTurnTime_temp))
        # Retrieving column index from column name.
        index_column_keepFlag = eventList_DF_5_cleanedTurnTime_temp.columns.get_loc('keepFlag')
        print('\nCombinning events with less than {} minutes turnTime difference...'.format(turnTime_tolerance))
        if isVerbose:
            print('Before combinning, total records is {}.'.format(len(eventList_DF_5_cleanedTurnTime_temp)))
        i_index = 1
        while(i_index < len(eventList_DF_5_cleanedTurnTime_temp)):
            if(eventList_DF_5_cleanedTurnTime_temp['diff'].iloc[i_index] <= timedelta(minutes=turnTime_tolerance)):
                cluster_begin_temp = i_index - 1
                while((eventList_DF_5_cleanedTurnTime_temp['diff'].iloc[i_index] <= timedelta(minutes=turnTime_tolerance)) ):
                    i_index += 1
                    if (i_index > len(eventList_DF_5_cleanedTurnTime_temp)-1):
                        break
                ...
                min_residue_diff_index_temp = eventList_DF_5_cleanedTurnTime_temp['residue_diff'].iloc[cluster_begin_temp:cluster_end_temp+1].idxmin()
                # Set record with min_residue as true, others as false.
                eventList_DF_5_cleanedTurnTime_temp.iloc[cluster_begin_temp:cluster_end_temp+1, index_column_keepFlag] = False
                eventList_DF_5_cleanedTurnTime_temp.loc[min_residue_diff_index_temp, 'keepFlag'] = True
            else:
                # For .iloc, can only use integer to specify row and column. No column string allowed.
                eventList_DF_5_cleanedTurnTime_temp.iloc[i_index, index_column_keepFlag] = True
                i_index += 1
        # Remove the records labeled as false.
        eventList_DF_5_cleanedTurnTime_temp = eventList_DF_5_cleanedTurnTime_temp[eventList_DF_5_cleanedTurnTime_temp['keepFlag']==True]
        eventList_DF_5_cleanedTurnTime_temp.reset_index(drop=True, inplace=True)
        # Drop diff.
        eventList_DF_5_cleanedTurnTime_temp = eventList_DF_5_cleanedTurnTime_temp.drop('diff', axis=1)
        # Reset index
        eventList_DF_5_cleanedTurnTime_temp.reset_index(drop=True, inplace=True)
        if isVerbose:
            print('After combinning, total records is {}.'.format(len(eventList_DF_5_cleanedTurnTime_temp)))
        print('Done.')

        # If eventList_DF_4_passWalenTest_temp is not empty, eventList_DF_5_cleanedTurnTime_temp can not be empty.
        # So we don't need to check if eventList_DF_5_cleanedTurnTime_temp is empty.

        print('\nCleaning events with bad fitting curve...')
        if isVerbose:
            print('Before cleaning, total records is {}.'.format(len(eventList_DF_5_cleanedTurnTime_temp)))
        # Clean the records with bad fitting curve shape.
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_5_cleanedTurnTime_temp.copy()
        # Calculate fitting coefficients, remove the events with high tail, high tail diff, high std.
        # Add a column 'lowTail'.
        eventList_DF_6_fineFittingCurve_temp['lowTail'] = [None]*len(eventList_DF_6_fineFittingCurve_temp)
        #eventList_DF_6_fineFittingCurve_temp['lowTail_value'] = [-999999]*len(eventList_DF_6_fineFittingCurve_temp)
        eventList_DF_6_fineFittingCurve_temp['lowTailDiff'] = [None]*len(eventList_DF_6_fineFittingCurve_temp)
        #eventList_DF_6_fineFittingCurve_temp['lowTailDiff_value'] = [-999999]*len(eventList_DF_6_fineFittingCurve_temp)
        eventList_DF_6_fineFittingCurve_temp['lowStd'] = [None]*len(eventList_DF_6_fineFittingCurve_temp)
        #eventList_DF_6_fineFittingCurve_temp['lowStd_value'] = [-999999]*len(eventList_DF_6_fineFittingCurve_temp)
        for index, record in eventList_DF_6_fineFittingCurve_temp.iterrows():
            #print('index = {}'.format(index))
            ...
            eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowTail'] = lowTail
            eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowTailDiff'] = lowTailDiff
            eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowStd'] = lowStd
            
            #eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowTail_value'] = ((Func_Pt_A(A_tail) - min_Func_Pt_A_value)/(max_Func_Pt_A_value - min_Func_Pt_A_value))
            #eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowTailDiff_value'] = abs(A_tail1 - A_tail2)/(A_max - A_min)
            #eventList_DF_6_fineFittingCurve_temp.loc[index, 'lowStd_value'] = Pt_fit_std
            #print(eventList_DF_6_fineFittingCurve_temp)
        ...
        keepMask = (eventList_DF_6_fineFittingCurve_temp['lowTail']&eventList_DF_6_fineFittingCurve_temp['lowTailDiff']&eventList_DF_6_fineFittingCurve_temp['lowStd'])
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp[keepMask]
        eventList_DF_6_fineFittingCurve_temp.reset_index(drop=True, inplace=True)

        # Drop diff.
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowTail', axis=1)
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowTailDiff', axis=1)
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowStd', axis=1)
        #eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowTail_value', axis=1)
        #eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowTailDiff_value', axis=1)
        #eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.drop('lowStd_value', axis=1)
        
        # Reset index
        eventList_DF_6_fineFittingCurve_temp.reset_index(drop=True, inplace=True)
        if isVerbose:
            print('After cleaning, total records is {}.'.format(len(eventList_DF_6_fineFittingCurve_temp)))
        print('Done.')

        # 5) Check point 5.
        # After removing bad curve, check if eventList_DF_6_fineFittingCurve_temp is empty.
        if eventList_DF_6_fineFittingCurve_temp.empty:
            # eventList_DF_6_fineFittingCurve_temp is empty. Do nothing.
            print('\nDataFrame eventList_DF_6_fineFittingCurve_temp is empty!')
            print('Go the the next iteration!')
            continue
        
        # Clean other overlapped events, with turnTime differenc longer than 5 minutes.
        # Sort eventList_DF_6_fineFittingCurve_temp by endTime.
        eventList_DF_6_fineFittingCurve_temp = eventList_DF_6_fineFittingCurve_temp.sort_values(by='endTime')
        # Make a copy.
        eventList_DF_7_fineFittingCurve_copy_temp = eventList_DF_6_fineFittingCurve_temp.copy()
        # Use the interval scheduling greedy algorithm to remove the overlapes.
        print('\nRemoving overlapped events...')
        if isVerbose:
            print('Before removing, total records is {}.'.format(len(eventList_DF_6_fineFittingCurve_temp)))
        # Drop keepFlag.
        ...
        if isPrintIntermediateDF:
            ...
            print('\neventList_DF_5_cleanedTurnTime_temp:')
            print(eventList_DF_5_cleanedTurnTime_temp)
            print('\neventList_DF_6_fineFittingCurve_temp:')
            print(eventList_DF_6_fineFittingCurve_temp)
...
def get_more_flux_rope_info(data_DF, dataObject_or_dataPath, **kwargs):
    ...
    if isinstance(dataObject_or_dataPath, pd.DataFrame):
        ...
        print('\nYour input is a path. Load the dictionary data via this path.')
        search_result_no_overlap_DF = pickle.load(open(dataObject_or_dataPath, 'rb'))
    else:
...
def plot_time_series_data(data_DF, spacecraftID, output_dir, **kwargs):
    ...
    if spacecraftID=='WIND':
        ...
        Tp_beta_twin.set_ylim([0,1]) #Bata.
